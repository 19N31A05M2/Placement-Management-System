					...
					<table id="example" class="table table-striped" style="width:100%">
						<thead>
							<tr class="bg-warning">
								...
								<th>Passoutyear</th>
								<th>Selected Company's</th>
								<th>package</th>
							</tr>
						</thead>
						<tbody>
							<?php
								$drives="SELECT rollno,name,gender,branch,passoutyear,GROUP_CONCAT(DISTINCT companyname SEPARATOR ', ') as Placed_Companies ,MAX(package) as package FROM placed where passoutyear='$year' GROUP BY rollno;";
								...
								While($row=mysqli_fetch_array($drivesquery))
								{
									...
									echo "<td>".$row['Placed_Companies']."</td>";
...
$(document).ready(function() {
	...
	$('.edit').on('click', function () {
		...
		$('#package').val(data_row[3]);
		$('#passoutyear').val(data_row[4]).change();
		$('#btechp').val(data_row[6]);
		$('#interp').val(data_row[7]);
		$('#schoolp').val(data_row[8]);
		if(data_row[5]=="CSE IT ECE EEE MECH ANE"){
			$('.all').click();
		}else{
		 
			$('.branch').each(function () {
				if (data_row[5].includes($(this).val())) {
					$(this).click();
				}
				else{
		...
		$('#backlogs').val(data_row[9]);
		$('#hbacklogs').val(data_row[10]);
		$('#ps').val(data_row[11]).change();
		$('#sg').val(data_row[12]).change();
	...
	$('#update').click(function(e){
		 ...
		 $.ajax({  
                url:"adddrive.php",  
                method:"POST",  
                data:{update:1,id:id,insert:0,deleted:0,companyname:companyname,date:date,packages:packages,passoutyear:passoutyear,btechp:btechp,interp:interp,schoolp:schoolp,branch:branch,backlogs:backlogs,hbacklogs:hbacklogs,ps:ps,sg:sg},    
